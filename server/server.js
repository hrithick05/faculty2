...
// Get all faculty members - sorted by designation hierarchy
...
// Award micro-level contribution points to faculty
app.post('/api/faculty/award-micro-level-points', async (req, res) => {
  try {
    ...
    // Validate faculty exists and get current micro-level values
    const microLevelFields = [
    ...
    // Select all fields (including micro-level contribution fields)
    const { data: facultyData, error: facultyError } = await supabase
      .from('faculty')
      .select('*')
      .eq('id', facultyId)
    ...
    console.log('✅ Faculty found:', facultyData);

    // Get current values and prepare update data
    const updateData = {};

    // Calculate new values (add to existing)
    for (const field of microLevelFields) {
      const currentValue = facultyData[field] || 0;
      const pointsToAdd = parseInt(points[field]) || 0;
      updateData[field] = currentValue + pointsToAdd;
      console.log(`📊 ${field}: ${currentValue} + ${pointsToAdd} = ${updateData[field]}`);
    }
    ...
    if (updateError) {
      console.error('❌ Error updating faculty points:', updateError);
      throw new Error(`Failed to update faculty points: ${updateError.message}`);
    }

    console.log('✅ Micro-level points awarded successfully');
    console.log('📋 Updated faculty data:', updatedFaculty);
    ...
    res.json({
      ...
      updatedFields: Object.keys(points).filter(key => parseInt(points[key]) > 0),
      updatedFaculty: updatedFaculty[0]
    });
...
const server = app.listen(PORT, async () => {
  ...
  try {
    ...
    console.error('⚠️  Warning: Achievement system initialization had issues:', error.message);
