...
import MicroLevelContributionTable from '../components/MicroLevelContributionTable';
...
const Index = () => {
  ...
  // Persist table view selection in localStorage
  const [tableView, setTableView] = useState(() => {
    const savedView = localStorage.getItem('dashboardTableView');
    // Migrate old 'department' value to 'micro-level'
    if (savedView === 'department') {
      localStorage.setItem('dashboardTableView', 'micro-level');
      return 'micro-level';
    }
    return savedView || 'faculty'; // Default to 'faculty' if not set
  });
  ...
  // Save table view selection to localStorage
  useEffect(() => {
    localStorage.setItem('dashboardTableView', tableView);
    console.log('💾 Table view saved:', tableView);
  }, [tableView]);
  ...
  const loadFacultyData = async () => {
    try {
      ...
      const { sortByDesignation } = await import('../utils/sortByDesignation');
      const sortedData = sortByDesignation(data || []);
  ...
  // Separate target row from faculty data and filter to Computer Science only
  const targetRow = facultyData.find(f => f.id === 'TARGET');
  const nonTargetFaculty = facultyData.filter(f => f.id !== 'TARGET');

  // Filter faculty - only Computer Science department
  const filteredFaculty = nonTargetFaculty.filter(faculty => {
    // Only show Computer Science department
    const isComputerScience = faculty.department === 'Computer Science' || 
                               faculty.department === 'computer science' ||
                               faculty.department?.toLowerCase() === 'computer science';
    
    return isComputerScience;
  });

  // Calculate summary statistics - only Computer Science department
  const computerScienceFaculty = facultyData.filter(faculty => {
    const isComputerScience = faculty.department === 'Computer Science' || 
                               faculty.department === 'computer science' ||
                               faculty.department?.toLowerCase() === 'computer science';
    return isComputerScience;
  ...
  const totalFaculty = computerScienceFaculty.length;
  const totalPublications = computerScienceFaculty.reduce((sum, faculty) =>
    sum + (faculty.journalpublications || 0) + (faculty.bookpublications || 0), 0);
  const totalPatents = computerScienceFaculty.reduce((sum, faculty) => sum + (faculty.patents || 0), 0);
  const totalProjects = computerScienceFaculty.reduce((sum, faculty) => sum + (faculty.studentprojects || 0), 0);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-purple-900 dark:to-blue-900 p-4 sm:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Achievement Tables with View Selector */}
          ...
          <div className="p-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
              <h2 className="text-2xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">
                {tableView === 'faculty' 
                  ? '👥 Faculty Achievement Overview (60%)' 
                  : '📊 Department Achievement Overview - Micro Level Contribution (40%)'}
              </h2>
              <Select value={tableView} onValueChange={setTableView}>
                <SelectTrigger className="h-12 px-4 rounded-xl border-2 border-purple-200 focus:border-purple-500 focus:ring-purple-500 min-w-[320px] bg-gradient-to-r from-purple-50 to-pink-50">
                ...
                <SelectContent>
                  <SelectItem value="faculty">
                    <div className="flex items-center gap-2">
                      <span>👥</span>
                      <span>Faculty Achievement Overview (60%)</span>
                    </div>
                  </SelectItem>
                  <SelectItem value="micro-level">
                    <div className="flex items-center gap-2">
                      <span>📊</span>
                      <span>Department Achievement Overview - Micro Level Contribution (40%)</span>
                    </div>
                  </SelectItem>
                </SelectContent>
              </Select>
