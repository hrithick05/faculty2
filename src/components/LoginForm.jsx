    ...
    if (!verified) {
      ...
      try {
        ...
        const faculty = (facultyData || []).find((f) => {
          // Normalize names: remove extra spaces, normalize punctuation, handle common variations
          const normalizeName = (str) => {
            return (str || '')
              .toLowerCase()
              .replace(/\s+/g, ' ') // Replace multiple spaces with single space
              .replace(/\.\s*/g, '.') // Remove spaces after periods
              .replace(/dr\./g, 'dr.') // Normalize Dr. variations
              .trim();
          };
          ...
          const nameMatch = namesSimilar(name, f.name);
          const idMatch = (f.id || '').toLowerCase().trim() === facultyId.toLowerCase().trim();
