...
const HODReviewPanel = () => {
  ...
  const [selectedFacultyForPoints, setSelectedFacultyForPoints] = useState(null);
  const [microLevelPoints, setMicroLevelPoints] = useState({
    reducing_fee_defaulters: 0,
  ...
  // Fetch faculty list for awarding points
  ...
  // Handle opening award points dialog
  const openAwardPointsDialog = async () => {
    await fetchFacultyList();
    setAwardPointsDialog(true);
    // Reset form
    setSelectedFacultyForPoints(null);
    setMicroLevelPoints({
      reducing_fee_defaulters: 0,
      reducing_dress_code_defaulters: 0,
      timely_completion_work: 0,
  ...
  const handleAwardPoints = async () => {
    ...
    // Check if at least one point is being awarded
    const totalPoints = Object.values(microLevelPoints).reduce((sum, points) => sum + (points || 0), 0);
    if (totalPoints === 0) {
      toast({
        title: "Error",
        description: "Please enter at least one point to award",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/faculty/award-micro-level-points`, {
        ...
        body: JSON.stringify({
          facultyId: selectedFacultyForPoints,
          points: microLevelPoints,
          awardedBy: getCurrentUser()?.id || 'HOD',
          awardedByName: getCurrentUser()?.name || 'HOD'
        })
      });
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      {/* Award Micro-Level Points Dialog */}
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <Select value={selectedFacultyForPoints || ''} onValueChange={setSelectedFacultyForPoints}>
                ...
                <SelectContent>
                  {facultyList.map((faculty) => (
                    <SelectItem key={faculty.id} value={faculty.id}>
                      {faculty.name} ({faculty.id}) - {faculty.designation}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-purple-50 p-4 rounded-lg border border-blue-200">
              <div className="flex justify-between items-center">
                <span className="font-semibold text-gray-700">Total Points to Award:</span>
                <span className="text-2xl font-bold text-blue-700">
                  {Object.values(microLevelPoints).reduce((sum, points) => sum + (points || 0), 0)}
                </span>
              </div>
            </div>
            ...
            <div className="flex gap-3 justify-end">
              <Button
                variant="outline"
                onClick={() => setAwardPointsDialog(false)}
              ...
              <Button
                ...
                disabled={awardingPoints || !selectedFacultyForPoints}
                className="bg-gradient-to-r from-purple-500 to-pink-500 text-white hover:from-purple-600 hover:to-pink-600"
              >
                {awardingPoints ? (
                  <>
