...
export async function fetchFacultyData(): Promise<FacultyMember[]> {
  try {
    ...
    // Import and apply designation sorting
    const { sortByDesignation } = await import('../utils/sortByDesignation');
    const sortedData = sortByDesignation(data || []);
    
    console.log('Faculty data fetched successfully:', sortedData?.length, 'records');
    console.log('First record sample:', sortedData?.[0]);
    return sortedData;
  } catch (error) {
...
// This file only contains data fetching functions and configuration